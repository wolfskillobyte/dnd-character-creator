...
function setRaceDesc() {
  ...
  if (charRace === 'dragonborn') {
    ...
    raceDesc.textContent =
      'Elves are a magical people of otherwordly grace, living in the world but not entirely part of it';
    ...
    raceDesc.textContent =
      'The diminutive halfings survive in a world full of larger creatures by avoiding notice or, barring that, avoiding offense';
...
function setAlignDesc() {
  ...
  if (charAlign === 'lawful good') {
    ...
    alignDesc.textContent =
      'Prefer to steer clear of moral questions and don’t take sides, doing what seems best at the time';
...
function setTrait(traitQueryString, promiseResolved) {
  var traitName =
    promiseResolved.results[
      Math.floor(Math.random() * promiseResolved.results.length)
    ].name;
...
function saveChar() {
  ...
  if ($('#char-class').text() != '' && savedChars) {
    ...
    //will also occur if savedChars is somehow not instantiated to an array, which probably won't happen
    UIkit.notification({
...
function saveDupeCheck() {
  for (var i = 0; i < savedChars.length; i++) {
    if (
      savedChars[i].name ==
        $('#character-name').text().replace('Your character is ', '') &&
